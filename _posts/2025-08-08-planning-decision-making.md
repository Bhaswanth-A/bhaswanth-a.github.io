---
title: Planning and Decision Making
date: 2025-06-10 00:00:00 +0800
categories: [CMU MRSD, Robotics]
tags: [planning, manipulators, cbs, ecbs, xecbs, multi-robot]     # TAG names should always be lowercase
cmu_subsection: "Fall 2025"
author: <author_id>
mermaid: true
pin: false
math: true
image: /assets/images/ldr.png
---

*In progress*

# Blog

These are my notes as part of the [16-782 Planning and Decision-making in Robotics](https://www.cs.cmu.edu/~maxim/classes/robotplanning_grad/) class taken by [Dr. Maxim Likhachev](https://www.cs.cmu.edu/~maxim/).

- [Basics and Sampling-Based Motion Planning](https://bhaswanth-a.github.io/posts/path-planning/)
- [All About Search Algorithms](https://bhaswanth-a.github.io/posts/all-about-search-algorithms/)
- [Case Studies of Planning and Decision-Making in Robotics](https://bhaswanth-a.github.io/posts/planning-case-studies/)


# Project: Accelerating Search-Based Planning for Multi-Robot Manipulation by Leveraging Online-Generated Experiences



